...
// Estado de conservación: única fuente de verdad para la condición del inmueble.
// Códigos válidos: 1, 2, 3, 4 y 6 — el 5 se salta a propósito para casar con
// Fotocasa. NULL = se desconoce. El 5 es un código heredado que sigue en base de
// datos hasta que corra la migración: se lee, pero ya no se escribe.
// Cualquier otro código no pinta fila. En particular el 0, el 7 y el 8 NO son
// estados de conservación: son el ordinal de la calificación energética que una
// importación antigua volcó aquí por error (8 = Exento), y pintarlos afirmaría
// sobre el estado del inmueble algo que sale de su certificado energético.
const CONSERVATION_STATUS_LABEL: Record<number, string> = {
  1: "Bueno",
  2: "Muy bueno",
  3: "Como nuevo",
  4: "A reformar",
...
function conservationStatusLabel(value: unknown): string | undefined {
  const code = Number(value);
  return Number.isFinite(code) ? CONSERVATION_STATUS_LABEL[code] : undefined;
}
...
export function PropertyCharacteristics({
  ...
  // undefined si el código no es uno de los que sabemos leer: entonces la fila no se pinta.
  const conservationLabel = conservationStatusLabel(property.conservationStatus);
        ...
        {(hasValue(property.propertyType) ||
          ...
          <div>
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              ...
              {!isLand && !!conservationLabel && (
                <div className="flex items-center justify-between rounded-lg bg-muted p-3">
                  <span className="font-medium">Estado conservación</span>
                  <span className="text-sm">{conservationLabel}</span>
                </div>
              )}
            </div>
          </div>
