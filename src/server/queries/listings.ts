...
function buildSortOrder(
  ...
  return [
    // La portada antepone los destacados a todo lo demás; el orden que la
    // agencia configura sigue mandando *dentro* de cada grupo. Sin esto, una
    // cuenta con `defaultSort` distinto de `"default"` —`newest`, `price-*`,
    // `size-*`, que no miran `isFeatured`— acababa enseñando en "Propiedades
    // destacadas" los 12 anuncios más recientes, destacados o no.
    ...(featuredFirst ? [desc(listings.isFeatured)] : []),
    ...primary(),
  ...
  async (limit = 12, accountIdArg?: bigint): Promise<ListingCardData[]> => {
    ...
    try {
      ...
      const listingsData = await db
        ...
        // Mismo orden que el listado, pero con los destacados por delante: la
        // sección se llama "Propiedades destacadas" y sólo caben `limit`
        // tarjetas, así que lo marcado por la agencia entra primero y el orden
        // configurado decide dentro de cada grupo. La rejilla ya filtra a
        // casa/piso, así que el agrupado por tipo no cambia nada aquí.
        .orderBy(...buildSortOrder(defaultSort, { featuredFirst: true }))
        .limit(limit);
